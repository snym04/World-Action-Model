...
class WanVideoVAE(nn.Module):
    ...
    def tiled_decode(self, hidden_states, device, tile_size, tile_stride, progress_bar=True):
        ...
        for h, h_, w, w_ in tqdm(tasks, desc="VAE decoding", disable=not progress_bar):
    ...
    def tiled_encode(self, video, device, tile_size, tile_stride, progress_bar=True):
        ...
        for h, h_, w, w_ in tqdm(tasks, desc="VAE encoding", disable=not progress_bar):
    ...
    def encode(self, videos, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16), progress_bar=True):
        ...
        for video in videos:
            ...
            if tiled:
                ...
                hidden_state = self.tiled_encode(video, device, tile_size, tile_stride, progress_bar=progress_bar)
            else:
    ...
    def decode(self, hidden_states, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16), progress_bar=True):
        hidden_states = [hidden_state.to("cpu") for hidden_state in hidden_states]
        videos = []
        for hidden_state in hidden_states:
            hidden_state = hidden_state.unsqueeze(0)
            if tiled:
                video = self.tiled_decode(hidden_state, device, tile_size, tile_stride, progress_bar=progress_bar)
            else:
