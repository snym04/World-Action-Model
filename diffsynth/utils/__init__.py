...
def resolve_cuda_device(device):
    """Normalize a device spec to a torch.device with an explicit index.

    `torch.cuda.mem_get_info` requires the device to carry an explicit index;
    it raises on plain "cuda" / torch.device("cuda"). This helper fills in the
    current CUDA device index when one is missing.
    """
...
class BasePipeline(torch.nn.Module):
    ...
    def get_vram(self):
        return torch.cuda.mem_get_info(resolve_cuda_device(self.device))[1] / (1024 ** 3)
        ...
        if self.path is None:
            ...
            if not skip_download:
                ...
                needs_download = (
                    ...
                    else not os.path.isdir(os.path.join(self.local_model_path, self.model_id))
                )
                if needs_download:
