...
class WanVideoPipeline(BasePipeline):
    ...
    def enable_usp(self):
        ...
        try:
            ...
            raise ImportError("USP requires the distributed module which is not included in this package.")
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if dit.seperated_timestep and fuse_vae_embedding_in_latents:
        ...
        num_temporal = latents.shape[2]
        t_per_token_list = []
        for b in range(B):
            ts_b = timestep[b] if timestep.dim() >= 1 and timestep.shape[0] > 1 else timestep
            t_tokens = torch.concat([
                torch.zeros((1, spatial_tokens), dtype=latents.dtype, device=latents.device),
                torch.ones((num_temporal - 1, spatial_tokens), dtype=latents.dtype, device=latents.device) * ts_b
            ]).flatten()
            t_per_token_list.append(t_tokens)
        t_per_token = torch.stack(t_per_token_list, dim=0)  # (B, num_tokens)
        t = dit.time_embedding(sinusoidal_embedding_1d(dit.freq_dim, t_per_token.reshape(-1)).reshape(B, -1, dit.freq_dim))
